...
int get_exp(decimal value) { return (value.bits[3] << 10) >> 26; }
int get_sign(decimal value) { return (value.bits[3] >> 31) & 1; }
  ...
  if (decreased < 10) {
    if (val_num) {
      if (((getBit(value_1->bits[0], 0) || more_than_once ||
        ...
        add(*value_1, (decimal){{1, 0, 0, value_1->bits[3]}}, value_1);
      }
    } else {
      if (((getBit(value_2->bits[0], 0) || more_than_once ||
        ...
        add(*value_2, (decimal){{1, 0, 0, value_2->bits[3]}}, value_2);
      }
    }
  }
}
...
int big_mul_by_10(big_decimal *value) {
  int res = 0;
  big_decimal value_2 = (big_decimal){
      {value->bits[0] << 1, (value->bits[1] << 1) | getBit(value->bits[0], 31),
      ...
      0};
  big_decimal value_8 = (big_decimal){
      {value->bits[0] << 3, (value->bits[1] << 3) | (value->bits[0] >> 29),
...
int add(decimal value_1, decimal value_2, decimal *result) {
  ...
  *result = (decimal){{0, 0, 0, 0}};
  ...
  if (get_sign(value_1) == get_sign(value_2)) {
    ...
    if (is_greater_or_equal(value_2, value_1)) {
...
int sub(decimal value_1, decimal value_2, decimal *result) {
  ...
  *result = (decimal){{0, 0, 0, 0}};
  ...
  if (get_sign(value_1) == get_sign(value_2)) {
    if (get_sign(value_1)) {
      if ((is_less_or_equal(value_1, value_2))) {
        basic_sub(value_1, value_2, result, &res);
      } else {
        basic_sub(value_2, value_1, result, &res);
        (*result).bits[3] = toggleBit((*result).bits[3], 31);
...
int basic_add(decimal value_1, decimal value_2, decimal *result, int *res) {
  ...
  if (carry) {
    ...
    if ((getBit(result->bits[0], 0) && (last == 5)) || (last > 5) ||
      ...
      add(*result, (decimal){{1, 0, 0, result->bits[3]}}, result);
    }
  }
  (*result).bits[3] =
...
int long_mul(decimal value_1, decimal value_2, decimal *result) {
  int exp = 0;
  *result = (decimal){{0, 0, 0, 0}};
  big_decimal long_result = (big_decimal){{0, 0, 0, 0, 0, 0}, 0};
  big_mul(
      (big_decimal){
...
void big_mul(big_decimal value_1, big_decimal value_2, big_decimal *result) {
  *result = (big_decimal){{0, 0, 0, 0, 0, 0}, 0};
  for (int i = 0; i < 96; i++) {
    int bit1 = getBit(value_1.bits[i / 32], i % 32);
    if (bit1) {
      big_decimal value_to_add = value_2;
      for (int j = 0; j < i; j++) {
...
void basic_mul(decimal value_1, decimal value_2, decimal *result) {
  *result = (decimal){{0, 0, 0, 0}};
  for (int i = 0; i < 96; i++) {
    int bit1 = getBit(value_1.bits[i / 32], i % 32);
    if (bit1) {
      decimal value_to_add = value_2;
      for (int j = 0; j < i; j++) {
        shift(&value_to_add);
      }
      value_to_add.bits[3] = 0;
      add(*result, value_to_add, result);
    }
  }
}
...
int div(decimal value_1, decimal value_2, decimal *result) {
  ...
  if (is_zero(value_2)) {
    ...
    *result = (decimal){{0, 0, 0, 0}};
    if (!is_zero(value_1)) {
      while (is_less(
          (decimal){{value_1.bits[0], value_1.bits[1], value_1.bits[2], 0}},
          (decimal){{value_2.bits[0], value_2.bits[1], value_2.bits[2], 0}})) {
...
int remainder_div(decimal value_1, decimal value_2, decimal *result) {
  ...
  *result = (decimal){{0, 0, 0, 0}};
  ...
  while (is_greater_or_equal(value_1, value_2)) {
    ...
    decimal tmp = value_2;
    int q = 0;
    while (is_greater_or_equal(value_1, value_to_add) && q < max_shift &&
    ...
    value_to_add = (decimal){{0, 0, 0, 0}};
    value_to_add.bits[(q - 1) / 32] = ((unsigned)1 << ((q - 1) % 32));
    add(*result, value_to_add, result);
    sub(value_1, tmp, &value_1);
    while (!(is_zero(value_1)) && is_less(value_1, value_2) && (cur_exp < 28) &&
           (!(result->bits[2] >> 28))) {
      mul_by_10(&value_1);
      mul_by_10(result);
      cur_exp++;
...
void round_after_division(decimal *result, int *cur_exp, int max_shift,
  ...
  decimal copy = *result;
  big_decimal big_result = (big_decimal){
  ...
  while (is_greater_or_equal(value_1, value_2)) {
    ...
    big_decimal value = (big_decimal){{0, 0, 0, 0, 0, 0}, 0};
...
int multiply_10_big(big_decimal *src) {
  big_decimal result = {{0}, 0};
  decimal value_2 = {{10, 0, 0, 0}};
  int check = 0;
...
int is_greater(decimal value_1, decimal value_2) {
  big_decimal value_1_big, value_2_big;
...
int d_floor(decimal value, decimal *result) {
  ...
  *result = (decimal){{0, 0, 0, 0}};
  ...
  if (remainder != 0) {
    decimal one = {{0x00000001, 0x00000000, 0x00000000, 0x00000000}};  // 1
    if (get_sign(value)) sub(*result, one, result);
  }
  return 0;
}

int d_round(decimal value, decimal *result) {
